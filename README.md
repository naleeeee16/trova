...
The bot and API are deployed to the cloud. No local setup is needed to use the bot — see [Using the Bot](#using-the-bot--user-guide).
...
The API runs as a Docker container. The bot runs as a background process on the same VM.

**1. Provision a cloud VM** (any provider — Railway, Render, GCP, DigitalOcean, etc.)

Required specs:
- 2 GB RAM minimum (embedding model + corpus)
- Python 3.11+ or Docker

**2. Copy the corpus and secrets to the server**

```bash
# Copy the normalized corpus (not in git)
...
**4. Start the bot (background process)**
...
Or use a process manager like `systemd` or `supervisor` to keep the bot running after disconnects.
...
This usually means all candidates failed the hard filter. The most common cause: one member has a very strict constraint (e.g., `must_be_free + specific music taste`) that no event satisfies. Try relaxing constraints or check logs:
```bash
